...
export default function TuitionPaymentsPage() {
  ...
  const getDueDateForSchedule = (dueMonth: number, academicYear?: string) => {
    ...
    return new Date(year, dueMonth - 1, 1); // 1er du mois comme rep̬re
  };
  ...
  const getScheduleStatus = (studentStatus: StudentPaymentStatus) => {
    ...
    // R̩partir le total pay̩ sur les ̩ch̩ances dans l'ordre
    ...
    if (!nextSchedule) {
      // Tous les schedules sont couverts mais solde non nul (cas de coh̩rence)
  ...
  const handleDownloadPaymentReceipt = async (payment: TuitionPayment) => {
    ...
    try {
      ...
      const receiptData = {
        ...
        school_address: schoolData?.address || 'Bingerville (Cefal apr̬s Adjam̩-Bingerville)',
        school_phone: schoolData?.phone || '+225 0707905958',
        academic_year: selectedStudent.tuitionFee?.academic_year || '',
        recorded_by: user?.full_name || 'Secr̩taire',
      };
      ...
      if (!response.ok) {
        throw new Error('Erreur lors de la g̩n̩ration du re̤u');
      }
      ...
      toast.error(error.message || 'Erreur lors du t̩l̩chargement');
    } finally {
